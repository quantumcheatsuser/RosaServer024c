...
sol::table* particleDataTables[maxNumberOfParticles];
...
void event::createBullet(int bulletType, Vector* pos, Vector* vel, int itemID) {
	int itm = -1;
	if (itemID) {
		itm = itemID;
	}
    subhook::ScopedHookRemove remove(&Hooks::createEventBulletHook);
    Engine::createEventBullet(bulletType, pos, vel, itm);
}
...
Bullet* bullets::create(int type, Vector* pos, Vector* vel, Player* ply) {
	...
	int bulletID = Engine::createBullet(type, pos, vel, ply == nullptr ? -1 : ply->getIndex());
	return bulletID == -1 ? nullptr : &Engine::bullets[bulletID];
}

int particles::getCount() { return *Engine::numParticles; }

sol::table particles::getAll() {
	auto arr = lua->create_table();
	for (int i = 0; i < *Engine::numParticles; i++) {
		auto body = &Engine::particles[i];
		arr.add(body);
	}
	return arr;
}

Particle* particles::getByIndex(sol::table self, unsigned int idx) {
	if (idx >= *Engine::numParticles)
		throw std::invalid_argument(errorOutOfRange);
	return &Engine::particles[idx];
}

int bonds::getCount() { return *Engine::numBonds; }

//sol::table bonds::getAll() {
//	auto arr = lua->create_table();
//	for (int i = 0; i < *Engine::numBonds; i++) {
...
sol::table bonds::getAll() {
	auto arr = lua->create_table();
	for (unsigned int i = 0; i < *Engine::numBonds; i++) {
		auto bond = &Engine::bonds[i];
		arr.add(bond);
	}
	return arr;
...
Building* buildings::getByIndex(sol::table self, unsigned int idx) {
	if (idx >= *Engine::numBuildings) throw std::invalid_argument(errorOutOfRange);
...
uintptr_t memory::getAddressOfParticle(Particle* address) {
	return (uintptr_t)address;
}
...
Item* EarShot::getTransmittingItem() const {
	return transmittingItemID == -1 ? nullptr: &Engine::items[transmittingItemID];
}
...
void Human::update() const {
	subhook::ScopedHookRemove remove(&Hooks::createEventUpdateHumanHook);
	Engine::createEventUpdateHuman(getIndex());
}
...
Bone* Human::getBone(unsigned int idx) {
	if (idx > 14) throw std::invalid_argument(errorOutOfRange);

	return &bones[idx];
}

Bone* Human::getRigidBody(unsigned int idx) {
	if (idx > 14) throw std::invalid_argument(errorOutOfRange);
...
void Human::setVelocity(Vector* vec) {
    for (int i = 0; i < 15; i++) {
        bones[i].vel.set(vec);
    }
}

void Human::addVelocity(Vector* vec) {
    for (int i = 0; i < 15; i++) {
        bones[i].vel.add(vec);
    }
}
...
void Item::update() const {
	subhook::ScopedHookRemove remove(&Hooks::createEventUpdateItemHook);
	Engine::createEventUpdateItem(getIndex());
}

void Item::updateInfo() const {
...
void Vehicle::applyDamage(int damage) const {
	subhook::ScopedHookRemove remove(&Hooks::vehicleApplyDamageHook);
	Engine::vehicleApplyDamage(getIndex(), damage);
}
...
//RigidBody* Vehicle::getRigidBody() const { return nullptr; } //&Engine::bodies[bodyID]; }

Particle* Vehicle::getParticle(unsigned int idx) const {
	if (idx >= numParticles) throw std::invalid_argument(errorOutOfRange);

	return &Engine::particles[particles[idx]];
}
...
Vehicle* Particle::getVehicle(unsigned int idx) const {
	if (idx >= numVehicles) throw std::invalid_argument(errorOutOfRange);

	return &Engine::vehicles[vehicles[idx]];
}

std::string Particle::__tostring() const {
	char buf[16];
	sprintf(buf, "Particle(%i)", getIndex());
	return buf;
}

int Particle::getIndex() const {
	return ((uintptr_t)this - (uintptr_t)Engine::particles) / sizeof(*this);
}

sol::table Particle::getDataTable() const {
	int index = getIndex();

	if (!particleDataTables[index]) {
		particleDataTables[index] = new sol::table(lua->lua_state(), sol::create);
	}

	return *particleDataTables[index];
}
...
int Bond::getBody() const { return bodyID; }

int Bond::getOtherBody() const { return otherBodyID; }

Human* Bond::getHuman() const {
	return &Engine::humans[humanID];
}
