...
#define SUBROSA_PREFIX "\033[31;1;4m[Sub Rosa 24C]\033[0m "
...
extern sol::table* particleDataTables[maxNumberOfParticles];
...
namespace event {
	...
	void createBullet(int bulletType, Vector* pos, Vector* vel, int item);
};  // namespace event
...
namespace physics {
	...
	sol::object lineIntersectTriangle(Vector* outPos, Vector* normal, Vector* posA,Vector* posB, Vector* triA, Vector* triB, Vector* triC, sol::this_state s);
...
namespace buildings {
	...
	//Building* getByName(const char* name);
};  // namespace vehicles
...
namespace vehicles {
	...
	Vehicle* createVel(int type, Vector* pos, Vector* vel, RotMatrix* rot, int color);
};  // namespace vehicles
...
int getCount();
sol::table getAll();
Particle* getByIndex(sol::table self, unsigned int idx);
...
uintptr_t getAddressOfParticle(Particle* address);
